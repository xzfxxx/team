...
- **成本与隔离理性**：多 agent ≈ 15× token，先算账再开；按任务选上下文隔离档位——**多 agent 的本质是隔离上下文**，能 lean 就别全量喂。
...
team-mode/
  ...
  │     ├── data-mapper.md         #  数据 / HANA + MyBatis 生成
  │     ├── mysql-mapper.md        #  MySQL 建表 + MyBatis 映射生成（本地 MySQL，无 gen-mapper 手生）
...
| **成本感知门** | 多 agent ≈ **15× token**；任务够大/能明显并行/隔离收益高才开，否则降级单 agent |
| **上下文隔离档位** | 简单子任务**只传指令**、复杂**全量委托**、共享状态走**文件/看板记忆**——省 token 也符合"隔离上下文"原则 |
| **防电话游戏直通** | 面向用户的结论由角色**直通**；worker 回主 agent 只给**浓缩摘要** |
| **收敛防线** | **卡死/模仿/跑偏**三哨兵 + 多质量师**加权裁决**，防 sycophancy |
| **产物契约** | 每角色有明确**输入/输出产物**，交接像外包合同，可单独验收 |
